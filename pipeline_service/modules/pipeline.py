...
class GenerationPipeline:
    ...
    def _edit_images(self, image: Image.Image, seed: int) -> list[Image.Image]:
        ...
        There are two multiview strategies:
        - base_multiseed (round11-style): one base prompt, multiple seeds
        - explicit_views (sn17_2-style): distinct prompts per view (left/right/back, etc.)
        """

        # Multiview disabled → single base view
        ...
        # Multiview enabled
        qwen_mode = getattr(self.settings.qwen, "mode", "base_multiseed")

        if qwen_mode == "explicit_views":
            logger.info("Multiview mode: explicit view prompts (sn17_2-style)")
            # Load views prompting library lazily so we don't require it in all configs
            from config.prompting_library import PromptingLibrary

            views_library = PromptingLibrary.from_file(self.settings.qwen.prompt_path_views)
            views_prompt = views_library.promptings.get("views")
            if not views_prompt or not views_prompt.prompt:
                logger.warning("Views prompting not found or empty, falling back to base_multiseed")
            else:
        ...
        # Fallback / default: base_multiseed (round11-style)
        logger.info("Multiview mode: 1 base prompt × 2 seeds for multidiffusion")
        base_prompt = self.prompting_library.promptings['base']
